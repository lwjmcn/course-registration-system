...
<head>
  ...
  <title>Admin Update StudentState Page</title>
...
<div class="content">
  ...
  %>
  <h3 class="subtitle">> 학생 정보 조회 및 변경 > 상태 변경</h3>
  <p style="color:#0a53be;font-weight:bold">학생 정보(학번): <%=studentID%></p>
  <!--상태 변경-->
  <div>
    <form method="post" action="UpdateStateAction.jsp">
      <div class="btn-group" data-roggle="buttons">
        <label class="btn btn-outline-dark">
          <input type="radio" name="state" autocomplete="off" value="재학" checked> 재학
        </label>
        <label class="btn btn-outline-dark">
          <input type="radio" name="state" autocomplete="off" value="휴학" checked> 휴학
        </label>
        <label class="btn btn-outline-dark">
          <input type="radio" name="state" autocomplete="off" value="자퇴" checked> 자퇴
        </label>
      </div>
